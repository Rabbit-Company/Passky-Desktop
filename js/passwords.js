        ...
        if(readData('passwords') !== null && typeof(readData('passwords')) !== 'undefined'){
            ...
            for(let i = 0; i < passwords.length; i++){
                ...
                if(passwords[i].message != null && typeof(passwords[i].message) != 'undefined'){
                    note = CryptoJS.AES.decrypt(passwords[i].message, decryptPassword(readData('password'))).toString(CryptoJS.enc.Utf8);
                }
                let password = CryptoJS.AES.decrypt(passwords[i].password, decryptPassword(readData('password'))).toString(CryptoJS.enc.Utf8);
                const data = passwords[i].id + ";;;" + passwords[i].website + ";;;" + passwords[i].username + ";;;" + password + ";;;" + note;
...
function addPassword(){
    ...
    password = CryptoJS.AES.encrypt(password, decryptPassword(readData('password'))).toString();
    message = CryptoJS.AES.encrypt(message, decryptPassword(readData('password'))).toString();
    ...
    xhr.onreadystatechange = function () {
        ...
        if(xhr.readyState === 4){
            ...
            if(json['error'] != 0){
                changeDialog(2, errors[readData('lang')][json['error']]);
                return;
            }
