loadData().then(() => {
  ...
  if (server !== null && typeof server !== "undefined") {
    let servers = [...document.getElementById("passky-server2").options].map(
      (v) => v.value
    );
  ...
  if (
    readData("username") !== null &&
    typeof readData("username") !== "undefined"
  )
    document.getElementById("username").value = readData("username");
  ...
  document.getElementById("forgot_username").innerText =
    lang["forgot_username"];
});
...
function changeDialog(style, text) {
  switch (style) {
    case 1:
      //Error dialog
      document.getElementById("dialog-icon").className =
      ...
      document.getElementById("dialog-button").className =
        "dangerButton inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium focus:outline-none sm:w-auto sm:text-sm";
    ...
    case 2:
      ...
      let server = readData("url");
      if (server !== null && typeof server !== "undefined") {
        let servers = [...document.getElementById("fu_server2").options].map(
          (v) => v.value
        );
        if (servers.includes(server)) {
      ...
      document.getElementById("dialog-button").className =
        "primaryButton inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium focus:outline-none sm:w-auto sm:text-sm";
      document.getElementById("dialog-button").innerText = lang["send"];
      document.getElementById("dialog-button").onclick = () =>
        forget_username();

      document.getElementById("dialog-button-cancel").onclick = () =>
        hide("dialog");
      break;
    case 3:
      ...
      document.getElementById("dialog-text").innerText =
        lang["email_sent_success"];

      document.getElementById("dialog-button-cancel").style.display = "none";

      document.getElementById("dialog-button").className =
        "successButton inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium focus:outline-none sm:w-auto sm:text-sm";
      document.getElementById("dialog-button").innerText = lang["okay"];
      document.getElementById("dialog-button").onclick = () => hide("dialog");
      break;
    case 4:
      ...
      document.getElementById("dialog-icon").className =
        "mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-blue-100 sm:mx-0 sm:h-10 sm:w-10";
      document.getElementById("dialog-icon").innerHTML =
        "<svg class='h-6 w-6 text-blue-600 animate-spin' xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' stroke-width='1.5' stroke='currentColor' aria-hidden='true'><path stroke='none' d='M0 0h24v24H0z' fill='none'></path><path d='M12 3a9 9 0 1 0 9 9'></path></svg>";
...
function login_check() {
  ...
  Argon2id.hash(
    authHash,
    Blake2b.hash("passky2020-" + username),
    32,
    32,
    4,
    64
...
function signin(url, username, authPassword, password, otp) {
  Passky.getToken(url, username, authPassword, otp)
    .then((response) => {
      ...
      Argon2id.hash(
        passHash,
        Blake2b.hash(username + "-passky2020"),
        32,
        32,
        4,
        64
...
function forget_username() {
  ...
  Passky.forgotUsername(url, email)
    .then((response) => {
      showDialogButtons();
